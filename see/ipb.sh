...
diable_blocklist_out() {
	...
	key_word=$(wget --no-check-certificate -t3 -T5 -qO- "https://raw.githubusercontent.com")
	[[ -z ${key_word} ]] && echo -e "${Error} 网络文件内容为空或访问超时 !" && display_out_keyworld && exit 0
	key_word_num=$(echo -e "${key_word}"|wc -l)
...
# Update_Shell(){
# 	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
